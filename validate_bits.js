...
// Check for orphaned entries outside the closing };
// Any "BIT-XXX": after the last }; is a deploy-breaking error
const lastBrace = dirSrc.lastIndexOf('};');
const afterBrace = dirSrc.slice(lastBrace + 2).trim();
if (/"BIT-[0-9]/.test(afterBrace)) {
  fail(`_bits_directives.js: bit entries found AFTER closing "};" — deploy will fail. Append landed outside the object.`);
} else {
  ok('Directives: all entries inside closing brace');
}
...
const BANNED = [
  ...
  "that makes sense", "I'm here", '"phew"', "'phew'",
  '"ha"', "'ha'", '"ha —"', "'ha —'",
  ...
  '*laughs*', '*pauses*', '*sighs*',
  "it's a whole thing", "a whole thing", "whole thing",
  "unrelated reasons",
  "morally soluble",
  "it's just a thing", "just a thing",
  "go on.", "Go on.", "go ahead.", "Go ahead.",
