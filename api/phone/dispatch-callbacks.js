...
// dispatch at most ONE job per run AND skip entirely if a job is already in
// flight ('dialing'), so overlapping cron ticks can't start a second agent.
//
...
async function agentBusy() {
  const url =
    `${SUPABASE_URL}/rest/v1/callback_jobs?status=eq.dialing&limit=1&select=id`;
...
async function markJob(jobId, status, failReason) {
  const body = { p_job_id: jobId, p_status: status };
  if (failReason) body.p_fail_reason = failReason;
...
export default async (req, res) => {
  ...
  try {
    ...
    if (!gate.ok) {
      await markJob(job.id, 'failed', gate.reason || 'can_dial_declined');
    ...
    try {
      ...
      await markJob(job.id, 'failed', `dispatch_error: ${String(e.message || e).slice(0, 120)}`);
